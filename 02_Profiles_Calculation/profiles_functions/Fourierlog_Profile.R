fourierlog_profile = function() {
  ...
  background_week_1 = log(link_data$background[1:(1440*7)])
  ...
  for (i in 2:length_weeks$total_weeks) {
    ...
    Fourier_profile[(1+(1440*7*(i-1))):(1440*7*i)] = exp(Re(fft(fourier_spectrum_filtered[(1+(1440*7*(i-2))):(1440*7*(i-1))], inverse = TRUE)))
    
    ## Get filtered spectrum of current week
    background_week_i = log(link_data$background[(1+(1440*7*(i-1))):(1440*7*i)])
