stllog_profile = function(use_background_and_spikes, automatic_settings_day, automatic_settings_week, automatic_settings_spike,
  ...
  STLlog_spikes_profile = rep(-1, (1440*7*length_weeks$pred_weeks))
  STL_profile = rep(-1, (1440*7*length_weeks$pred_weeks))
  
  for (i in 1:length_weeks$pred_weeks) {
    print(paste("Calculating STL LOG Profile. Current week: ", i,"/",length_weeks$pred_weeks,"."))
    
    ## Get traveltime and background for 8 weeks of training
    travel_time_train <- log(link_data$travel_time[(1+(1440*7*(i-1))):(1440*7*(i+length_weeks$span_weeks))])
    background_train <- log(link_data$background[(1+(1440*7*(i-1))):(1440*7*(i+length_weeks$span_weeks))])
    ...
    if(compute_linear_trend == TRUE) {
      STL_background_profile[(1+(1440*7*(i-1))):(1440*7*i)] <- exp(trend_background_pred) + exp(seasonal_background_pred)
      ...
      STL_background_profile[(1+(1440*7*(i-1))):(1440*7*i)] <- exp(median(trend_background_pred)) + exp(seasonal_background_pred)
    ...
    if (use_background_and_spikes == TRUE) {
      ...
      spikes_train <- log(link_data$spikes[(1+(1440*7*(i-1))):(1440*7*(i+length_weeks$span_weeks))])
      spikes_train[is.infinite(spikes_train)]=0
      # Measurements per seasonal (week) cycle
      freq_spike = 10080
      ...
      #seasonal_spikes_pred[is.infinite(spikes_train)]=-Inf
      
      ## Get spikes STL profile prediction
      STLlog_spikes_profile[(1+(1440*7*(i-1))):(1440*7*i)]  <- exp(seasonal_spikes_pred)
      #STLlog_spikes_profile[(1+(1440*7*(i+length_weeks$span_weeks))):(1440*7*(i+length_weeks$train_weeks))] <- seasonal_spikes_pred
      STL_profile[(1+(1440*7*(i-1))):(1440*7*i)] <- STL_background_profile[(1+(1440*7*(i-1))):(1440*7*i)] + STLlog_spikes_profile[(1+(1440*7*(i-1))):(1440*7*i)]
      #STL_profile[(1+(1440*7*(i+length_weeks$span_weeks))):(1440*7*(i+length_weeks$train_weeks))] <- STL_background_profile[(1+(1440*7*(i+length_weeks$span_weeks))):(1440*7*(i+length_weeks$train_weeks))] + STLlog_spikes_profile[(1+(1440*7*(i+length_weeks$span_weeks))):(1440*7*(i+length_weeks$train_weeks))]
    } else {
